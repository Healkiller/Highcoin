...
static bool IsCanonicalSignature(const valtype &vchSig) {
    ...
    // Where R and S are not negative (their first byte has its HIGHTest bit not set), and not
    ...
    if (!CKey::CheckSignatureElement(S, nLenS, true))
        return error("Non-canonical signature: S value is unnecessarily HIGHT");
